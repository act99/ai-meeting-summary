...
try:
    console.print("🚀 Whisper Large 모델 다운로드 중...", style="bold blue")
    console.print("💡 정확도 96% - 비용 $0 (완전 무료)", style="green")
    
    # 모델 다운로드 (자동으로 캐시됨)
    model = whisper.load_model("large")
    
    console.print("✅ Whisper Large 모델 다운로드 완료!", style="bold green")
    console.print("🎯 이제 비용 없이 최고 정확도로 음성 인식을 사용할 수 있습니다!", style="yellow")
    
    # 모델 정보 출력
    console.print("\n📊 모델 정보:", style="bold blue")
    console.print(f"   - 모델 크기: Large (1.5GB)")
